...
if (!window._navimanClickListenerAttached) {
    ...
    *******************************************************/  
    ...
    /*******************************************************
    Fix cho việc nếu publish 1 grid/mega menu dạng publishToPlace thì 
    sẽ gán z-index đến mức cha là .naviman_app
    *******************************************************/     

    window.addEventListener("load", function() {
        document.querySelectorAll('.naviItem.publishToPlace').forEach(child => {
            const parent = child.closest('.naviman_app');
            console.log(".........");
            console.log(parent);
            if (parent) {
            const childZIndex = window.getComputedStyle(child).zIndex;
            if (childZIndex !== 'auto') {
                parent.style.zIndex = childZIndex;
                console.log(`✅ Set z-index ${childZIndex} to parent of .publishToPlace`);
            }
            }
        });
    });
