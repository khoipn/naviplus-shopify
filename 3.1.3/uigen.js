...
var generateCSS_UI_Level1_Background = function ( setting, cssNaviPrefix, section_setting ) {
    ...
    if( getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) == NAVIGLOBAL['MENU_KINDS']['STICKY_HAMBURGER'] 
        || getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) == NAVIGLOBAL['MENU_KINDS']['SECTION_MEGAMENU'] )
        {
...
var generateCSS = function(naviman_appItem, embed_id, setting, dragdrop, isNaviSection, section_setting) {
    ...
    if( // Is sticky menu
        menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_MOBILE_TABBAR'] ||
        menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_MOBILE_HEADER'] ||
        menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_HAMBURGER'] ||
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_HAMBURGER'] )        
...
var generateCSS_FixForMegaMenu_Desktop = function(cssNaviPrefix, setting, dragdrop, section_setting) {
  var addHtml = "";
  addHtml += cssNaviPrefix.trim() + '.SECTION_MEGAMENU' + ' ul li ul.children { top: ' + parseInt(setting['height']) + 'px;} ';
  addHtml += cssNaviPrefix.trim() + '.SECTION_MEGAMENU' + ' ul.children { width: ' + parseInt(setting['submenuWidth']) + 'px;} ';
...
var fixMegaMenu = function (naviman_appItem, shop, embed_id, section_setting) {
    ...
    if( getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) != NAVIGLOBAL['MENU_KINDS']['SECTION_MEGAMENU'] )
        return;
...
var hideNotStickySection = function (naviman_app, naviItem, embed_id, section_setting) {
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MEGAMENU'] ) {
        if (isMobileMode()) {
            console.log("Hide SECTION_MEGAMENU on mobile");
            var session = getSessionParent(naviman_app);
            session.style.display = "none";
        }
    }*/
        ...
        response.forEach((naviItem) => {
            ...
            if( isDisplayed ) {
                if (isHadValue(naviItem["data"]["setting"]['publishToPlaceDisplay'])) {
                    if (naviItem["data"]["setting"]['publishToPlaceDisplay'] == "0") {
            ...
            console.log( naviItem["embed_id"] + " display: " + isDisplayed );
...
var fixBottomBar_showLevel3Items_Desktop = function(menuItem, isNaviSection, menuKind, embed_id) {
  ...
  if(menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_MOBILE_TABBAR'] ) { 
