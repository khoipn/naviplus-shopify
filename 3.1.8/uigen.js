 ...
 * https://cdn.jsdelivr.net/gh/khoipn/naviplus-shopify/${version.trim()}/uigen.min.js
...
var itemMedia =  function(icon, image, naviman_domain, style='', iconboxpadding, /*iconBoxPaddingTop = 0, */itemExtIconSize = '', itemExtAlign = '', seoUrl = '', seoName = '')
{
    ...
    if (!isHadImage) {
        ...
        var output = '<div class="image-border" ' + itemExtAlign + ' ><span class="image-box" ' + style + '><span class="image">' + '<img title="' + seoName + '" alt="' + seoName + '" ' + styleImageHeight + ' src="' + naviman_json_files + image + '">' + '</span></span></div>';
        if (seoUrl != '') output = seoUrl + output + '</a>';
        return output;
    }
};
...
var getClassOfMenuKind = function( naviman_shopinfo, embed_id ) {
    ...
    switch (menuKind) {
        case NAVIGLOBAL['MENU_KINDS']['STICKY_TABBAR']:
            return "STICKY_TABBAR";
        ...
        case NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU']:
            return "SECTION_DESKTOP_MEGAMENU";


        case NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE']:
            return "CONTEXT_SLIDE";

    }
}

var isStickyMenu = function(menuKindClass) {
    if( menuKindClass == "STICKY_TABBAR" ) return true;
    if( menuKindClass == "STICKY_MOBILE_HEADER" ) return true;
    if( menuKindClass == "STICKY_FAB_SUPPORT" ) return true;
    return false;
}
...
function checkBrowserSupport() {
    ...
    if (window._browserSupportChecked) {
        // console.log('Browser support has already been checked.');
        return;
    }
...
/** uigen/libs.js END ****************************************/
    ...
    class Helper {
  static isUrlContain(keywordsSetting) {
...
var generateCSS_UI_Level1_Menuitems = function ( setting, cssNaviPrefix, naviman_appItem, dragdrop, section_setting ) {
    ...
    if (isHadValue(setting['fontFamily'])) {
        //let fontFamily_css = '<style> @import url("https://fonts.googleapis.com/css2?family=' + setting['fontFamily'] + ':wght@400;500;600;700&display=swap"); </style>';
...
var generateCSS_UI_Level1_Background = function ( setting, cssNaviPrefix, section_setting ) {
    ...
    if( getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) == NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE'] 
        || getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] )
        {
...
var generateCSS = function(naviman_appItem, embed_id, setting, dragdrop, isNaviSection, section_setting) {
    ...
    var menuKind = getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id);
    ...
    if( 
        ...
        html += generateCSS_FixForMegaMenu_Mobile(cssNaviPrefix, setting, dragdrop, isNaviSection, section_setting);

    if(  menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] ) 
        html += generateCSS_FixForMegaMenu_Desktop(cssNaviPrefix, setting, dragdrop, isNaviSection, section_setting);        
    ...
    var generateCSS_FixForMegaMenu_Mobile = function(cssNaviPrefix, setting, dragdrop, section_setting) {
  var addHtml = "";
  addHtml += cssNaviPrefix + ' ul li ul.children { top: ' + parseInt(setting['height']) + 'px !important;} ';
  return addHtml;
};

var generateCSS_FixForMegaMenu_Desktop = function(cssNaviPrefix, setting, dragdrop, section_setting) {
  var addHtml = "";
...
var fixMegaMenu = function (naviman_appItem, shop, embed_id, section_setting) {
    ...
    if( getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id) != NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] )
        return;
...
var fixNotStickyParents = function (naviman_app, naviItem, embed_id, section_setting) {
    ...
    if( session != null ) {
        ...
        menuKind = getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id);
...
var hideNotStickySection = function (naviman_app, naviItem, embed_id, section_setting) {
    var menuKind = getMenuKind(navihelper.windowVar.get('shopinfo'), embed_id);

    // Cái này không hoạt động đúng
    /* if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_GRID'] ) {
        if (!isMobileMode()) {
            console.log("Hide SECTION_MOBILE_GRID on desktop");
            var session = getSessionParent(naviman_app);
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_HEADER'] ) {
        if (!isMobileMode()) {
            console.log("Hide SECTION_MOBILE_HEADER on desktop");
            var session = getSessionParent(naviman_app);
            session.style.display = "none";
        }
    }
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_MEGAMENU'] ) {
        if (!isMobileMode()) {
            console.log("Hide SECTION_MOBILE_MEGAMENU on desktop");
            var session = getSessionParent(naviman_app);
            session.style.display = "none";
        }
    }

    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_BANNER'] ) {
        if (!isMobileMode()) {
            console.log("Hide SECTION_MOBILE_BANNER on desktop");
            var session = getSessionParent(naviman_app);
            session.style.display = "none";
        }
    } */
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] ) {
        if (isMobileMode()) {
            console.log("Hide SECTION_DESKTOP_MEGAMENU on mobile");
            var session = getSessionParent(naviman_app);
            session.style.display = "none";
        }
    }*/
    ...
    var drawBottomNav = function (response, naviman_domain, var_shop, var_embed_id, section_setting = []) {
        ...
        // Nếu ở dạng section thì bọc bên ngoài một the div embed_id + "-container" và điều chỉnh việc hiển thị -------------------
        ...
        var generateMenu_Children_LIList = function(children, naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses = "", childrenLevel = 2) {
    var index = 0;
    ...
    children.forEach((child) => {
              ...
              if (desktopHover) {
                  if (childIsParent) {
                      if (!(childExtClassname.includes('navi-nohover') || childExtClassname.includes('navi-hover'))) {
                          ...
                          childOnClick += 'onMouseEnter="clearTimeout(naviman.hoverTimeout); naviman.hoverTimeout = setTimeout(() => { if (this.matches(\':hover\')) { this.classList.add(\'navi-hover-active\'); ' + onhoverFunc + '; } }, 250);" ';
                          childOnClick += 'onMouseLeave="clearTimeout(naviman.hoverTimeout); naviman.hoverTimeout = setTimeout(() => { if (!this.matches(\':hover\')) { this.classList.remove(\'navi-hover-active\'); ' + onhoverFunc + '; } }, 250);" ';
                      }
                  }
              }            
      ...
      */
      ...
      if(isOptimizeSEO(navihelper.windowVar.get('shopinfo')))
          seoUrl = '<a alt="'+ child["name"] +'" title="'+ child["name"] +'" href="'+ getSEOUrl( childUrl ) +'" onclick="return false;">';
      ...
      if( childrenLevel == 3 ) 
        ...
        html += generateMenu_Children_Level4(
            child["children"], naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses, 4
        );
      }
...
var generateMenu_Children = function(children, naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses = "", childrenLevel = 2) {
    ...
    html+= generateMenu_Children_LIList(children, naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses, childrenLevel);
    html += '</ul>';
  return html;
}
...
var generateMenu_Children_Level4 = function(children, naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses = "", childrenLevel = 2) {
        
    var html = '<li class="divider-level4-top" >&nbsp;<li>';
    html+= generateMenu_Children_LIList(children, naviItem, data, extSubmenu, isNaviSection, section_setting, otherClasses, 4);
...
var generateMenu_Children_FullExpand = function(children, naviItem, data, extSubmenu, isNaviSection, section_setting, fullexpandWidth = "") {
  var index = 0;
  ...
  if (fullexpandWidth != "") {
    fullexpandWidth = standardizePXValue(fullexpandWidth);
    if (fullexpandWidth != "") {
      fullWidthPxStyle = `width: ${fullexpandWidth}; overflow: auto; left: 50%; transform: translate(-50%, 0px);`;
    }
  }
...
var generateMenu = function( data, naviItem, isNaviSection, section_setting ){
    ...
    data["dragdrop"].forEach((item) => {
                ...
                if (desktopHover) {
                    if (itemIsParent) {
                        if (!(itemExtClassname.includes('navi-nohover') || itemExtClassname.includes('navi-hover'))) {
                            // Khai báo hoverTimeout trong phạm vi của naviman
                            itemOnClick += 'onMouseEnter="clearTimeout(naviman.hoverTimeout); naviman.hoverTimeout = setTimeout(() => { if (this.matches(\':hover\')) { this.classList.add(\'navi-hover-active\'); ' + onhoverFunc + '; } }, 100);" ';
                            itemOnClick += 'onMouseLeave="clearTimeout(naviman.hoverTimeout); naviman.hoverTimeout = setTimeout(() => { if (!this.matches(\':hover\')) { this.classList.remove(\'navi-hover-active\'); ' + onhoverFunc + '; } }, 100);" ';
                        }
                    }
                }
        ...
        */
        ...
        var seoUrl = '';
        if(isOptimizeSEO(navihelper.windowVar.get('shopinfo')))
            seoUrl = '<a alt="'+ item["name"] +'" title="'+ item["name"] +'" href="'+ getSEOUrl( itemUrl ) +'" onclick="return false;">';
        ...
        html += '<li ' +
            ...
            + '" ' + itemOnClick + ' data-name="' + item["name"] + '" data-kind="' + itemKind + '"' + 'data-fullexpand="'+  getExtVariable(item, "fullexpand", "0") +'"' + '>';
        ...
        if (itemIsParent) 
            if( getExtVariable(item, "fullexpand", "") == "1" ) {
                html += generateMenu_Children_FullExpand(item["children"], naviItem, data, 
                    ...
                    isNaviSection, section_setting, defaultValue(data["setting"]['submenuFullExpandWidth'], "") );    
            }else {
                var childrenLevel = 2;
                html += generateMenu_Children(
                    item["children"], 
...
var isItemSMS = function ( item_url ) {
    ...
    return  false;
        ...
        response.forEach((naviItem) => {
            ...
            // Mỗi lần bắt đầu một menu item thì sẽ reset về dạng section hoặc không
            ...
            var menuKindClass = getClassOfMenuKind(navihelper.windowVar.get('shopinfo'), naviItem["embed_id"]);

            var isDisplayed = true;

            // ƯU TIÊN 1: Lọc ẩn theo Keyword  -----------------------------------------------------------------------------------               
            ...
            if (isHadValue(naviItem["data"]["setting"]['publishContainKeyword'])) {
                isDisplayed = Helper.isUrlContain(naviItem["data"]["setting"]['publishContainKeyword']);
                if( isDisplayed == false )
                    console.log( naviItem["embed_id"] + " is hide because DONOT contain keyword: " + naviItem["data"]["setting"]['publishContainKeyword']);
            }
            
            if (isHadValue(naviItem["data"]["setting"]['publishDontContainKeyword'])) {
                isDisplayed = !Helper.isUrlContain(naviItem["data"]["setting"]['publishDontContainKeyword']);
                if( isDisplayed == false )
                    console.log( naviItem["embed_id"] + " is hide because contain keyword: " + naviItem["data"]["setting"]['publishDontContainKeyword']);
            }
            ...
            // ƯU TIÊN 2.1: STICKY MENU: Chỉ hiển thị displayGlobal được bật  -----------------------------------------------------
            if( isDisplayed ) 
                if( isStickyMenu(menuKindClass) ) {
            ...
            // ƯU TIÊN 2.2: CONTEXT MENU: Luôn luôn được bật  -----------------------------------------------------
            ...
            if( menuKindClass == "CONTEXT_SLIDE" ) {
                ...
                console.log( naviItem["embed_id"] + " is always shown. The cause is: CONTEXT_SLIDE" );
            }
            

            // ƯU TIÊN 2.3: PUBLISH TO PLACE Luôn luôn được bật  -----------------------------------------------------
            ...
            if( isPublishToPlace == true ) {
                publishToPlace = " publishToPlace ";
                isNaviSection = true;
            }   
            
            
            if( isPublishToPlace == true ) 
                console.log( naviItem["embed_id"] + " will publiced to place: " + naviItem["data"]["setting"]['publishToPlace'] );

            if( isPublishToPlace == true )
                isDisplayed = true;

            // ƯU TIÊN 3: PLATFORM Mobile & Desktop  ----------------------------------------------------------------------
            ...
            if( isDisplayed == false ) 
                console.log( naviItem["embed_id"] + " is hide. The cause is did not active for this platform (mobile/desktop) " );

            // ƯU TIÊN 4: BACKEND MODE: môi trường thiết kế backend thì luôn hiện để dễ theo dõi  -------------------------
            if (isBackendMode())
                isDisplayed = true;

            if( isDisplayed == false ) 
                console.log( naviItem["embed_id"] + " is always shown. The cause is in the backend env " );

            
            console.log( ">> " + naviItem["embed_id"] + " (" +  menuKindClass + ") display: " + isDisplayed );
            ...
            if (isDisplayed) {
                ...
                var divNaviItem = '<div style="visibility: hidden;'+ embedMarginStyle +'" '+ attribute +' name="' + naviItem["name"] + '" id="'+ naviItem["embed_id"] +
                    '" class="naviItem ' + publishToPlace + naviItem["embed_id"] + menuPositionClass( data ) + ' ' + 
                    /*getClassOfMenuKind(navihelper.windowVar.get('shopinfo'),naviItem["embed_id"])  */ menuKindClass
                    + '"></div>';
            ...
            // == Fix lỗi cho cả hiển thị hoặc không hiển thị ===========================<<
            if( embed_id != "" && (section_setting['not_sticky'] == true || section_setting['not_sticky'] == "true" ) ) {
                hideNotStickySection( naviman_app, response[0], embed_id, section_setting );
            }
            // == Fix lỗi cho cả hiển thị hoặc không hiển thị ===========================>>
...
var fixBottomBar_showLevel3Items_Desktop = function(menuItem, isNaviSection, menuKind, embed_id) {
  ...
  if(menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_TABBAR'] ) { 
    ...
    var leftEdgeOfLevel1 = ulChildrent.parentElement.parentElement.parentElement.parentElement.getBoundingClientRect().left;
...
var gotoUrl = function (event, item, is_parent, url, embed_id, isNaviSection) {
    ...
    if (!is_parent) {
        ...
        if (item.classList.contains("child")) {
            showLevel3Items_Mobile(item, isNaviSection, menuKind, embed_id);
...
var showLevel3Items_Mobile = function (menuItem, isNaviSection, menuKind, embed_id) {
...
var showLevel3Items_Desktop = function (menuItem, isNaviSection, menuKind, embed_id) {
    ...
    if (!isMobileMode) {
        ...
        fixBottomBar_showLevel3Items_Desktop(menuItem, isNaviSection, menuKind, embed_id);
    }
}
...
var getSettingOfNaviman = function(embed_id) {
    var navimanObj = null;
        for (var i = 0; i < navimanData.length; i++)
            if (navimanData[i]["embed_id"] == embed_id) {
...
var showLevel2Items = function (menuItem, menuKind, embed_id) {
    ...
    if( desktopHover ) {
        let now = Date.now();    
        if (now - lastShowLevel2ItemsCall < 200) {
        ...
        if (!is_showing) {
            ...
            if( menuItem.getAttribute('data-fullexpand') == "1" ) {
                ...
                if( isFrom_PublishToPlaceMenu(menuItem, embed_id) )
                    maintainFullExpandDropdownPosition(ulChildrent);
            }
            else
    ...
    for (let navi_item = 0; navi_item < naviItems.length; navi_item++) {
        ...
        for (let i = 0; i < listItems.length; i++) {
            ...
            if (url) { // Kiểm tra nếu url không null
                ...
                if (url !== "" && topContain(pathName, url)) {
...
function turnOffLocalStorage() {
    ...
    if (window.__localStorageDisabled) {
    ...
    window.__localStorageDisabled = true;
}


function isLocalStorageSupported() {
    // Nếu kết quả đã được lưu, trả về ngay
    if (typeof window.__localStorageSupport !== 'undefined') {
        return window.__localStorageSupport;
    }
    ...
    try {
        ...
        window.__localStorageSupport = true; // Lưu kết quả vào window
    } catch (e) {
        window.__localStorageSupport = false; // Lưu kết quả vào window
    }
...
function isSessionStorageSupported() {
    // Nếu kết quả đã được lưu, trả về ngay
    if (typeof window.__sessionStorageSupport !== 'undefined') {
        return window.__sessionStorageSupport;
    }
    ...
    try {
        ...
        window.__sessionStorageSupport = true; // Lưu kết quả vào window
    } catch (e) {
        window.__sessionStorageSupport = false; // Lưu kết quả vào window
    }
...
setTimeout(() => {
    ...
    if( overlay_global ) {
        overlay_global.addEventListener("click", function (event) {
            ...
            document.querySelectorAll('.CONTEXT_SLIDE').forEach((item) => {
...
function maintainFullExpandDropdownPosition(dropdownFullExpand) {
    function updatePosition() {
        ...
        if (parent) {
            ...
            // console.log("Dropdown Before Update:", dropdownFullExpand.style);
            
            dropdownFullExpand.style.top = rect.bottom + "px";
            
            //console.log("Dropdown After Update:", dropdownFullExpand.style);
        } else {
