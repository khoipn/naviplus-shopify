      ...
      elsArray.forEach(function(elsIdClass, index) {
          ...
          els.forEach(function(el) {
              ...
              newEl.addEventListener("click", function(event) {
                    if (newEl.tagName.toLowerCase() === "a") {
                        event.preventDefault(); // chặn chuyển hướng link
                    }
      ...
      if( childShowBadgeMode == BADGE_ISCART_WITHCOUNT ) {
          ...
          if( cartCount == 0 || cartCount == "" ) { 
            ...
            if( cartCount < 10 )
                  ...
                  document.documentElement.style.setProperty('--cart-count-number', `"N"`);
          }
      }
        ...
        response.forEach((naviItem) => {
            ...
            if (isDisplayed) {
                ...
                var divNaviItem = '<div style="visibility: hidden;'+ embedMarginStyle +'" '+ attribute +' name="' + naviItem["name"] + '" id="'+ naviItem["embed_id"] +
                    '" class="naviItem ' + publishToPlaceClass + naviItem["embed_id"] + menuPositionClass( data ) + menuSlidePositionClass(data, menuKind) + menuSlideDesktopSubDirection(data, menuKind) + ' ' + 
                    menuKindClass
                    + '"></div>';
