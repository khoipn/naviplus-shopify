...
var debugModeLog = function(message) {
    if (isDebugMode()) {
        console.log(message);
    } else {
...
var debugModeLog = function(style, message) {
    if (isDebugMode()) {
...
Menu.Common.Level1.generateHeight = function(setting, cssNaviPrefix, menuKind) {
    ...
    if( menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] ||
        ...
        addHtml += cssNaviPrefix + ' ul.navigation { height: ' + setting['height'] + 'px } ';
    }
...
var generateMenu = function( data, naviItem, isNaviSection, section_setting, menuKind ){
    ...
    data["dragdrop"].forEach((item) => {
        ...
        */
        ...
        var seoUrl = '';
        if(isOptimizeSEO(navihelper.windowVar.get('shopinfo')))
            seoUrl = '<a alt="'+ naviLanguage.stringByLanguage(item["name"]) +'" title="'+ naviLanguage.stringByLanguage(item["name"]) +'" href="'+ getSEOUrl( itemUrl ) +'" onclick="return false;">';
        ...
        html += '<li ' +
            ...
            + '" ' + itemOnClick + ' data-name="' + naviLanguage.stringByLanguage(item["name"]) + '" data-kind="' + itemKind + '"' + 'data-fullexpand="'+  getExtVariable(item, "fullexpand", "0") +'"' + '>';
        ...
        response.forEach((naviItem) => {
            ...
            if (isDisplayed) {
                ...
                var divNaviItem = '<div style="visibility: hidden;'+ embedMarginStyle +'" '+ attribute +' name="' + naviItem["name"] + '" id="'+ naviItem["embed_id"] +
                    '" class="naviItem ' + publishToPlaceClass + naviItem["embed_id"] + menuPositionClass( data ) + ' ' + 
                    menuKindClass
                    + '"></div>';
