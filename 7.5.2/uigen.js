...
var naviman_json_cdn = "https://cdn.naviplus.app/naviplus/data/json";
var naviman_json_files = "https://cdn.naviplus.app/naviplus/data/";
...
class FixGoogleApp {
  constructor() {
    ...
    this._sessionKey = 'fixGoogleApp_restored'; // cờ lưu trong sessionStorage
  }
  ...
  fixNavimanApp() {
    ...
    // Nếu đã restore trong session này => bỏ qua luôn
    if (sessionStorage.getItem(this._sessionKey) === 'true') {
      miniSnackbar?.show?.('⏩ Skip FixGoogleApp (already restored this session)', 1500, 'info');
      return;
    }
    ...
    // 🔧 Ép layout full viewport để tránh sticky
    ...
    // 🎯 Khi user scroll hoặc chạm vuốt -> restore lại style gốc
    const restore = () => {
      ...
      // Ghi cờ vào sessionStorage để lần sau bỏ qua
  ...
  apply() {
    ...
    if (!this.isGoogleApp) return;

    miniSnackbar?.show?.(`Version: ${window.naviman_version ?? 'unknown'} running`, 1500, 'info');

    const run = () => this.fixNavimanApp();
...
/* class FixGoogleApp {
  ...
  apply() {
    if (!this.isGoogleApp) {
      miniSnackbar?.show?.('❌ Not in Google App', 1000, 'info');
      return;
    } else {
      miniSnackbar?.show?.(`Version: ${window.naviman_version ?? 'unknown'} running`, 1500, 'info');
    }
    ...
    const tryAddOverlay = () => {
      ...
      // Nếu overlay đã tồn tại thì thôi
      if (document.getElementById('__fixGoogleAppOverlay')) return;

      const overlay = document.createElement('div');
      overlay.id = '__fixGoogleAppOverlay';
      Object.assign(overlay.style, {
        ...
        background: 'rgba(0,0,0,0.1)', // 🔴 Dễ thấy để test
        display: 'block',
        touchAction: 'none',
        cursor: 'default',
      });
      ...
      const activate = () => {
        miniSnackbar?.show?.('🩵 Overlay touched — repaint done', 1000, 'info');
        overlay.remove();
      };

      overlay.addEventListener('touchstart', activate, { passive: true });
      overlay.addEventListener('pointerdown', activate, { passive: true });
    ...
    // Gọi lại liên tục cho tới khi body xuất hiện
    const watcher = setInterval(() => {
      if (document.body || document.documentElement) {
        tryAddOverlay();
        miniSnackbar?.show?.('🩵 Overlay touched', 1000, 'info');
...
if (!window.fixGoogleAppInitialized) {
  ...
  window.addEventListener('load', () => {
    setTimeout(() => fix.apply('.naviItem'), 2000);
  });
}
