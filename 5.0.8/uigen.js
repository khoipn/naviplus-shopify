...
var menuSlidePositionClass = function ( data, menuKind ) {
...
Helper.waitForCssToLoad = function(callback) {
...
Menu.Sticky.lockPageScrollingTabBar = function (menuKind, isLock) {    
    ...
    // console.log("lockPageScrollingTabBar", menuKind, isLock);
    ...
    if (isLock) {
        ...
        // Lưu trạng thái hiện tại nếu chưa có
        ...
        // Phục hồi style nếu có bản sao lưu
...
var generateCSS_UI_Common = function ( setting, cssNaviPrefix, naviman_appItem, dragdrop, section_setting ) {
    ...
    if( isUserLoggedIn() ) {
        addHtml += cssNaviPrefix + ' ul li.publish-show-logined { display: -webkit-inline-box }';
        ...
        addHtml += cssNaviPrefix + ' ul li.publish-hide-logined { display: -webkit-inline-box }';
    }
...
var generateMenu_Children_FullExpand = function(children, naviItem, data, extSubmenu, isNaviSection, section_setting, fullexpandWidth = "", menuKind) {
  var index = 0;
...
var generateMenu = function( data, naviItem, isNaviSection, section_setting, menuKind ){
    ...
    if( scrollMobileAllow ) {
        console.log("___Scroll mobile allow___");
        console.log(naviItem["embed_id"]);
        console.log(scrollMobileFixWidth);
        ...
        response.forEach((naviItem) => {
            ...
            if (isDisplayed) {
                ...
                var divNaviItem = '<div style="visibility: hidden;'+ embedMarginStyle +'" '+ attribute +' name="' + naviItem["name"] + '" id="'+ naviItem["embed_id"] +
                    '" class="naviItem ' + publishToPlaceClass + naviItem["embed_id"] + menuPositionClass( data ) + menuSlidePositionClass(data, menuKind) + ' ' + 
                    menuKindClass
                    + '"></div>';
    ...
    if (menuItem != null) {
        if (!is_showing) {
            ...
            var closeDropDown = function () {
                ...
                console.log("function: closeDropDown");
        ...
        setTimeout(() => {
            ...
            if( overlay_global ) {
                overlay_global.addEventListener("click", function (event) {                
                    console.log("Click to naviman_app_overlay_global");
